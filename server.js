...
function shouldBlockRequest(url) {
  try {
    ...
    return BLOCKED_URL_WORDS.some(x =>
      lower.includes(x)
    );
  } catch {
    return false;
...
async function searchNamu(
   ...
   *
   * 하나만 찾도록 되어 있어서
   * 나무위키의 DOM 구조가 조금만 달라져도
   *
   * locator.waitFor: Timeout 15000ms exceeded
   *
   * 오류가 발생했다.
   *
   * 이제 여러 selector를 순서대로 확인한다.
   * =======================================================
   */
  ...
  for (
    const selector of searchCandidates
  ) {
    const candidate =
      p.locator(
        selector
