...
async function downloadImage(originalUrl, index, total) {
  ...
  try {
    ...
    const existing = existingFiles.find(filename =>
...
async function searchNamu(
  ...
  // 나무위키 대문 검색창을 실제 UI처럼 사용한다.
  // 고정된 placeholder를 기준으로 검색창을 찾고,
  // 검색 버튼을 직접 클릭한다.
  await p.goto(
  ...
  if (!candidates.length && needle) {
    setProgress(
      ...
      base + 4,
      `주제 포함 결과가 없어 나무위키 검색창에서 '${keyword} ${topic}'을 다시 검색합니다`
    );
...
async function extractDocument(
  ...
  result.text = cleanAdText(
    result.text
  );

  for (const section of result.sections) {
    section.text = cleanAdText(
      section.text
    );
  }
  ...
  const savedAll =
    await downloadImages(rawImages);
...
function attachImages(data, docs) {
  ...
  function findImage(
    ...
    const source =
      normalizeSection(sourceSection);

    let found = allImages.find(
      image =>
        !used.has(image.localUrl) &&
        normalizeSection(
          image.sectionNumber
        ) === source
    );
  ...
  data.blogSections =
    Array.isArray(data.blogSections)
      ? data.blogSections
      : [];

  data.blogSections =
    data.blogSections.map(section => {
